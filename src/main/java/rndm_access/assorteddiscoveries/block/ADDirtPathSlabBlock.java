...
public class ADDirtPathSlabBlock extends SlabBlock {
    ...
    public static final MapCodec<ADDirtPathSlabBlock> CODEC;

    public ADDirtPathSlabBlock(Settings settings) {
        super(settings);
    }

    @Override
    public MapCodec<ADDirtPathSlabBlock> getCodec() {
        return CODEC;
    }
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState neighborState,
        ...
        if(direction.equals(Direction.UP) && !state.get(TYPE).equals(SlabType.BOTTOM)) {
            return ADBlocks.DIRT_SLAB.getDefaultState().with(TYPE, state.get(TYPE))
    ...
    static {
        ...
        SHAPE.put(SlabType.TOP, Block.createCuboidShape(0.0, 8.0, 0.0, 16.0, 15.0,
                16.0));
        CODEC = createCodec(ADDirtPathSlabBlock::new);
    }
}
